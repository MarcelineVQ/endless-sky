...
PlayerModesPanel::PlayerModesPanel(PlayerInfo &player, function<void()> callback)
...
void PlayerModesPanel::Draw() const
{
	if(player.IsDead())
		return;
...
bool PlayerModesPanel::KeyDown(SDL_Keycode key, Uint16 mod, const Command &command)
{
	if(isDeparting)
		return false;
	...
	if(key == 'd' && flagship && flagship->CanBeFlagship())
	{
		...
		else
		{
			...
			if(fighterCount > 0 || droneCount > 0 || cargoToSell > 0)
			{
				...
				GetUI()->Push(new Dialog(this, &PlayerModesPanel::TakeOff, out.str()));
				return true;
			}
			else
				TakeOff();
...
bool PlayerModesPanel::Click(int x, int y)
{
